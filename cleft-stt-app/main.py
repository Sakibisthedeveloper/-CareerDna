...
if keys_str:
    keys_pool.extend([k.strip() for k in keys_str.split(",") if k.strip()])

key_3 = os.getenv("GEMINI_API_KEY_3")
if key_3 and key_3.strip() not in keys_pool:
    keys_pool.append(key_3.strip())

if not keys_pool:
    for var in ["GOOGLE_AI_API_KEY", "GEMINI_API_KEY", "GOOGLE_API_KEY"]:
        val = os.getenv(var)
        if val:
            keys_pool.append(val.strip())

# Filter out any None values if a key is missing
...
if SUPABASE_URL and SUPABASE_KEY:
    try:
        ...
        logger.info("Supabase credentials found and client successfully initialized. Persistent cloud history enabled.")
...
def load_learned_phonetic_rules():
    """Load learned phonetic rules generated by the background learning agent."""
    global LEARNED_RULES
    if not os.path.exists(PHONETIC_RULES_PATH):
        return
    try:
        with open(PHONETIC_RULES_PATH, 'r', encoding='utf-8') as f:
            LEARNED_RULES = f.read().strip()
            if LEARNED_RULES:
                logger.info(f"Loaded learned phonetic rules dynamically on startup: {len(LEARNED_RULES.splitlines())} rules.")
    except Exception as e:
        logger.error(f"Error loading phonetic rules: {e}")

# Load rules upon startup
load_learned_phonetic_rules()
