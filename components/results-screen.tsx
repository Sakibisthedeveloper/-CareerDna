...
export function ResultsScreen() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('careerDnaResults')
      if (saved) {
  ...
  const resultType = localData?.type || "creative"
  const userName = localData?.name || "Explorer"
  ...
  useEffect(() => {
    ...
    if (matches.length > 0) {
      ...
      const winningScore = localData?.scores ? localData.scores[resultType] : 0
  ...
  }, [resultType, localData])

  if (!mounted) return null // Ensures immediate hydration match pulling only from localStorage on client side mount
      ...
      <main className="relative z-10 mx-auto max-w-4xl px-6 pt-28 pb-24">
        ...
        >
          {[
            ...
            const fullTypeMap: Record<string, string> = { 'c': 'creative', 'a': 'analytical', 's': 'social', 't': 'technical' }
            const score = localData?.scores ? localData.scores[fullTypeMap[item.key]] : 0
        ...
        {/* Action buttons */}
        <div
          className={`mb-16 flex flex-col items-center gap-4 transition-all delay-600 duration-700 sm:flex-row sm:justify-center ${revealed ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
            }`}
        >
          <Link
            href="/quiz"
        ...
        <div
          className={`mx-auto max-w-xl transition-all delay-700 duration-700 ${revealed ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
            }`}
        >
          <div className="overflow-hidden rounded-3xl border border-border/60 bg-card/60 p-8 backdrop-blur-xl shadow-2xl relative">
            <div className="absolute inset-0 bg-gradient-to-br from-[oklch(0.65_0.25_300/0.05)] to-transparent" />

            {!isSubmitted ? (
              <div className="relative z-10">
                <div className="mb-6 text-center">
                  <h3 className="text-2xl font-bold text-foreground mb-2">Save Your Results</h3>
                  <p className="text-sm text-muted-foreground">
                    Get your detailed breakdown sent to your inbox and save your progress.
                  </p>
                </div>
                    ...
                    try {
                      ...
                      const { error } = await supabase
                        ...
                        .insert({
                          ...
                          career_1: primaryMatch?.title,
                          career_2: altMatches[0]?.title,
                          career_3: altMatches[1]?.title,
                      ...
                      if (!error) {
                        setIsSubmitted(true);
                        // Update local name if possible
                        const updatedData = { ...localData, name: name || localData.name };
                        localStorage.setItem('careerDnaResults', JSON.stringify(updatedData));
                ...
                >
                  <div className="relative">
                    ...
                    <input
                      ...
                      required
                      className="w-full rounded-2xl border border-border/60 bg-background/50 py-4 pl-12 pr-4 text-sm transition-all focus:border-[oklch(0.65_0.25_300)] focus:outline-none focus:ring-1 focus:ring-[oklch(0.65_0.25_300)]"
                    />
                  </div>
                  <div className="relative">
                  ...
                  <button
                    ...
                    className="group relative w-full overflow-hidden rounded-full bg-gradient-to-r from-[oklch(0.65_0.25_300)] to-[oklch(0.72_0.19_55)] py-4 text-sm font-bold text-primary-foreground shadow-lg transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70"
                  >
                    <span className="absolute inset-0 bg-gradient-to-r from-[oklch(0.72_0.19_55)] to-[oklch(0.65_0.25_300)] opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
              ...
              <div className="relative z-10 py-4 text-center">
                <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[oklch(0.65_0.25_300/0.1)] text-[oklch(0.65_0.25_300)]">
                  <CheckCircle2 className="h-8 w-8" />
                </div>
                <h3 className="text-xl font-bold text-foreground mb-1">Results Saved!</h3>
                <p className="text-sm text-muted-foreground">
                  Thank you, {name}! Your CareerDNA has been saved to our database.
                </p>
              </div>
            )}
          </div>
